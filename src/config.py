...
# ===== 실험용 손잡이(knobs): 여기 값을 바꾸고 `python -m src.train` 로 'BLEND last-block' 확인 =====
# (주의: 아래 MODEL_WEIGHT_FLOOR·SHARPEN_T 는 train.py 에서만 적용. evaluate 는 빠른 미리보기라 미반영)
...
N_SPLITS = 5         # 시간블록 CV 폴드 수
SEEDS = [42, 7, 2024]  # 멀티시드 배깅용 LGBM 시드

# ===== '과감하게' 실험용 손잡이 (기본 중립). train.py 에 적용. =====
MODEL_WEIGHT_FLOOR = 0  # 블렌드에서 모델 최소 비중(0=자동, 0.8=prior 거의 안 섞고 모델 강제)
SHARPEN_T = 1.0           # 확률 샤프닝 온도: <1=더 극단(자신감↑), 1=그대로, >1=0.5쪽으로. 후보 0.6/0.8/1.2
